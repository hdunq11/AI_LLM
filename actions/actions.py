...
class ActionListFoodsByCategory(Action):

    def name(self) -> Text:
        return "action_list_foods_by_category"

    def run(self, dispatcher: CollectingDispatcher,
            tracker: Tracker,
            domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:

        food_category = next(tracker.get_latest_entity_values("food_category"), None)
        if not food_category:
            dispatcher.utter_message(text="Bạn muốn biết món ăn nào thuộc loại gì?")
            return []

        with open("actions/data/foods.json", "r", encoding="utf-8") as f:
            foods = json.load(f)

        category_foods = [food for food in foods if food['food_category'].lower() == food_category.lower()]
        if not category_foods:
            dispatcher.utter_message(text=f"Xin lỗi, không có món ăn nào thuộc loại {food_category}.")
            return []

        response = f"📝 Các món ăn thuộc loại {food_category}:\n"
        for food in category_foods:
            response += f"- {food['name']}\n"
        dispatcher.utter_message(text=response)
        return []
